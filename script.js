...
// WebSocket connection for voice commands
function initializeWebSocket() {
    const socket = new WebSocket('ws://localhost:8080');

    socket.addEventListener('message', (event) => {
        const message = event.data.toLowerCase().trim();
        console.log("WebSocket received:", message);

        if (message === "show dashboard") {
            window.open("index.html", "_blank");
        } else if (message === "patient progress") {
            window.open("patient_progress.html", "_blank");
        } else if (message === "show videos") {
            window.open("demonstration.html", "_blank");
        }
    });
}
